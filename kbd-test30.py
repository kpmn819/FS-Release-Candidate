...
def click_wait():
    ...
    while True:
        ...
        if event.type == pygame.KEYDOWN:
                    ...
                    click_spot =(-1, -1)
                    # might return a negative number here
...
def rotation_decode(enc_pin):
    ...
    if (Switch_A == 1) and (Switch_B == 0) : # A then B ->
        ...
        # set the status so it can be read the True needs to be set false elsewhere
        ...
        if enc_pin == enc4_pins[0]:
            enc4_stat = [True, 'up']
        # !!!!!!!!!!!!!!!!! THIS CODE IS HARDWIRED AND NEEDS TO REFLECT WHICH KNOB HAS TURNED !!!!!!!!!!!!!!!!!!!!!
